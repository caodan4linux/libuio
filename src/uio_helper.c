/*
   ...
   Copyright (C) 2020, Dan.Cao <caodan@linuxtoy.cn>
...
static struct uio_info_t* info_byname(char* name, const char *filter_name)
{
	struct uio_info_t* info;
	...
	if (line_from_file(filename, linebuf))
		return NULL;
	...
	if (!info)
		return NULL;
	memset(info,0,sizeof(struct uio_info_t));
	info->uio_num = uio_num_from_filename(name);
...
struct uio_info_t* uio_find_devices_byname(const char *filter_name)
{
	...
	while(n--) {
		infp = info_byname(namelist[n]->d_name, filter_name);
		free(namelist[n]);
		if (!infp)
			continue;
		if (!infolist)
